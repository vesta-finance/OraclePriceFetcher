...
export class DeploymentHelper {
	...
	async deployUpgradeableContract(
		...
		const [findOld, address] = this.tryToGetSaveContractAddress(identityName)
		...
		colorLog(Colors.green, `Deployed ${identityName} at ${contract.address}`)
	...
	tryToGetSaveContractAddress(contractName: string): [boolean, string] {
		if (this.deploymentState[contractName] !== undefined) {
			...
			colorLog(Colors.green, `${contractName} already exists. Loading ${address}`)
	...
	async sendAndWaitForTransaction(txPromise: Promise<any>) {
		...
		const minedTx = await ethers.provider.waitForTransaction(tx.hash, this.config.TX_CONFIRMATIONS)

		if (!minedTx.status) {
			throw `Transaction failed ${txPromise}`
		} else {
			colorLog(Colors.blue, `${minedTx.transactionHash} minted successfully`)
		}
